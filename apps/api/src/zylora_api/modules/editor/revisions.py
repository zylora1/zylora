...
class AiCreditService:
    ...
    async def account(self, user_id: UUID, *, lock: bool = False) -> AiCreditAccount:
        ...
        if created_user_id is not None:
            self.session.add(
                AiCreditLedger(
                    user_id=user_id,
                    operation_id=uuid4(),
                    delta=FREE_AI_ALLOWANCE,
                    entry_type="MONTHLY_GRANT",
                    resulting_balance=FREE_AI_ALLOWANCE,
                    reason="Initial Free monthly AI allowance",
                )
            )
        ...
        if account.period_end <= now:
            previous = account.balance
            start, end = _month_bounds(now)
            account.allowance = max(account.allowance, FREE_AI_ALLOWANCE)
            account.balance = account.allowance
            ...
            self.session.add(
                AiCreditLedger(
                    ...
                    reason="Monthly AI allowance reset",
                )
            )
        return account
